...
def test_deposit_success(
    ...
    with caplog.at_level(logging.DEBUG):
        ...
        uploaded_metadata = s3_instance.client.get_object(
            Bucket="awd", Key="10.1002-term.3131.json"
        )
        assert uploaded_metadata["ResponseMetadata"]["HTTPStatusCode"] == HTTPStatus.OK
        s3_instance.client.get_object(Bucket="awd", Key="10.1002-term.3131.pdf")
        assert uploaded_metadata["ResponseMetadata"]["HTTPStatusCode"] == HTTPStatus.OK
        ...
        assert (
            len(s3_instance.client.list_objects(Bucket="awd")["Contents"])
            == 3  # noqa: PLR2004
        )
